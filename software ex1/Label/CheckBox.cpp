...
CheckBox::CheckBox(string* textList, int listSize, short width, short hieght, DWORD color) {
	int row = hieght;
...
bool* CheckBox::GetChosenRows() {
	return chosen;
}
void CheckBox::HandleInput(INPUT_RECORD iRecord) {
	//if (!isClicked) return;
...
void CheckBox::MouseEventProc(MOUSE_EVENT_RECORD mer) {
...
void CheckBox::MousePressed(MOUSE_EVENT_RECORD mer) {
...
void CheckBox::MouseMoved(MOUSE_EVENT_RECORD mer) {
	int res = CheckPosition(mer);
	if (res != -1) {
		list[currentRow].Hoover(false);
		currentRow = res - startPoint.Y;
...
int CheckBox::CheckPosition(MOUSE_EVENT_RECORD mer) {
...
void CheckBox::KeyEventProc(KEY_EVENT_RECORD ker){
	if (ker.bKeyDown) {
